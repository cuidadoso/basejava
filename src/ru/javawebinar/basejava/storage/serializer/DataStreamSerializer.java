...
public class DataStreamSerializer implements StreamSerializer {
        ...
        try(DataOutputStream dos = new DataOutputStream(os)) {
            ...
            writeCollection(dos, sections.entrySet(), entry -> {
                ...
                switch(type) {
                    case PERSONAL:
                    case OBJECTIVE:
                        dos.writeUTF(((TextSection) s).getContent());
                        break;
                    case ACHIEVEMENT:
                    case QUALIFICATIONS:
                        writeCollection(dos, ((ListSection) s).getItems(), dos::writeUTF);
                        break;
                    case EXPERIENCE:
                    case EDUCATION:
                        writeCollection(dos, ((OrganizationSection) s).getOrganizations(), o -> {
                            dos.writeUTF(o.getHomePage()
                                          .getName());
                            dos.writeUTF(o.getHomePage()
                                          .getUrl());
    ...
    private Section readSection(final DataInputStream dis, final SectionType sectionType) throws IOException {
        switch(sectionType) {
            case PERSONAL:
            case OBJECTIVE:
            ...
            case EDUCATION:
                return new OrganizationSection(readList(dis, () -> new Organization(
                        ...
                        readList(dis, () -> new Organization.Position(readLocalDate(dis),
                                                                      readLocalDate(dis),
                                                                      dis.readUTF(),
                                                                      dis.readUTF()))
                )));
    ...
    private <T> List<T> readList(final DataInputStream dis, final ElementReader<T> reader) throws IOException {
        int size = dis.readInt();
